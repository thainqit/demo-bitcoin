$(function() {
  ...
  function calcEma(close, ema_1, length){
    var alpha = 2.0 / (length + 1);
    return alpha * close + (1 - alpha) * ema_1;
  }

  $.getJSON('/data?periods=' + selectTime, function (full_data) {
    ...
    for (i; i < dataLength; i += 1) {
      ...
      var currentEma = calcEma(data[i][4], lastEma, i + 1);
      ema.push([
        data[i][0], // the date
        currentEma // ema value
      ]);
      lastEma = currentEma;

      ichimoku.push([
        data[i][0], // the date
        (data[i][2] + data[i][4]) / 2 // ichimoku value
      ]);
    }
    ...
    window.setInterval(function(){
      var url = '/data?periods=' + selectTime + '&after=' + lastTime;
      $.getJSON(url, function (full_data) {
        ...
        if (lastDataLength === 1) {
          ...
          ema[ema.length -1] = [
            lastData[0][0],
            calcEma(lastData[0][4], ema[ema.length - 2][1], ema.length)
          ]
